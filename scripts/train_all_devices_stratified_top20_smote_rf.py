...
TRAIN_PATH = Path("data/splits/all_devices_train_stratified_top20.csv")
TEST_PATH = Path("data/splits/all_devices_test_stratified_top20.csv")
REPORT_DIR = Path("outputs/reports")
REPORT_PATH = REPORT_DIR / "all_devices_stratified_random_forest_top20_smote.txt"
...
def main() -> None:
    ...
    X_test_scaled = scaler.transform(X_test).astype("float32", copy=False)
    ...
    print("Training Random Forest with top20 + SMOTE...")
    ...
    summary = f"""All Devices Stratified Random Forest Top20 + SMOTE
========================================================

Dataset
-------
Train rows before SMOTE: {len(X_train)}
...
Test Distribution
-----------------
benign: {y_test.value_counts().sort_index().get(0, 0)}
attack: {y_test.value_counts().sort_index().get(1, 0)}
...
Scale seconds: {scale_seconds:.2f}
